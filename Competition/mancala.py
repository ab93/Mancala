...
class GameState:
    ...
    def setEvalScore(self):
        if MAX_PLAYER == 1:
            h1 = self.getP1_Mancala() - self.getP2_Mancala()
...
def AlphaBeta(State,player,timeLeft):
...
def GetLegalSuccessors(state,player):
    ...
    if player == 1:    #only for player 1
        ...
        return list(reversed(possibleStates)),list(reversed(nextMove))   #so that the pit near to P1's mancala is given priority
...
def ParseInputFile():
    ...
    with open(sys.argv[2]) as inputFile:
        ...
        task = inputFile.readline().strip()    #task = 4
        ...
        return A,B,float(timeLeft)


def main():
    A,B,timeLeft = ParseInputFile()

    global MAX_PLAYER,MIN_PLAYER

    Start = GameState(A,B)

    AlphaBeta(Start,MAX_PLAYER,timeLeft)
